...
He is suspicious of the email, but it does mention a major insurance association, and that makes him think it might be legitimate(合法). 
Which of the following best describes this attack?
A. Phishing (網路釣魚)
B. Social engineering (社交工程)
C. Spear phishing (魚叉式網路攻擊)(Spear Phishing : 特定對象)
D. Trojan horse (特洛伊木馬)

5. Juanita is a network administrator for a small accounting firm. 
The users on her network are complaining(抱怨) of (slow(慢) connectivity(連線)很慢的連線).
...
large number of half-open connections. == 半開放式連線
...
firm. 
After three months of using the product, one of your accountants(設計師) accidently discovers a way to
log in(登入) and bypass(繞過) all security and authentication(認證). 
